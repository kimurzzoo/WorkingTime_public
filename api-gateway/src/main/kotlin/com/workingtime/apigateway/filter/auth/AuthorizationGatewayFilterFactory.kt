...
@Component
class AuthorizationGatewayFilterFactory : AbstractGatewayFilterFactory<AuthorizationGatewayFilterFactory.Config>(Config::class.java) {
    ...
    override fun apply(config: Config): GatewayFilter {
        return GatewayFilter { exchange, chain ->
            ...
            if(jwtAuthorizationUtil.validateToken(token))
            {
                ...
                addAuthorizationHeaders(exchange.request, jwtAuthorizationUtil.getUsername(token), userInfo["role"] as String, config.role!!)
            }
            return@GatewayFilter chain.filter(exchange)
        }
    }

    private fun addAuthorizationHeaders(request : ServerHttpRequest, username : String, role : String, configrole : String)
    {
        if(configrole == role)
        {
